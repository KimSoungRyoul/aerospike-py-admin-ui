...
def _generate_digest(rand) -> str:
    ...
    return "".join(hex_chars[int(math.floor(rand() * 16))] for _ in range(40))
...
def _generate_user_records() -> list[AerospikeRecord]:
    ...
    for i in range(1, 51):
        ...
        age = 18 + int(math.floor(rand() * 52))
        active = rand() > 0.2
        user_tags = _pick_n(TAGS, 1 + int(math.floor(rand() * 4)), rand)

        metadata = {
            "signupDate": f"2025-{str(1 + int(math.floor(rand() * 12))).zfill(2)}-{str(1 + int(math.floor(rand() * 28))).zfill(2)}",
        ...
        records.append(
            AerospikeRecord(
                ...
                meta=RecordMeta(
                    generation=1 + int(math.floor(rand() * 10)),
                    ttl=-1,
                    lastUpdateMs=now_ms - int(math.floor(rand() * 86400000 * 30)),
                ),
                bins={
                    "name": f"{first} {last}",
                    "email": email,
    ...
    for i in range(1, 31):
        ...
        records.append(
            AerospikeRecord(
                ...
                meta=RecordMeta(
                    generation=1 + int(math.floor(rand() * 5)),
                    ttl=-1,
                    lastUpdateMs=now_ms - int(math.floor(rand() * 86400000 * 60)),
                ),
                bins={
                    "name": name,
                    "price": price,
...
def _generate_order_records() -> list[AerospikeRecord]:
    ...
    for i in range(1, 201):
        ...
        user_id = f"user-{str(1 + int(math.floor(rand() * 50))).zfill(4)}"
        status = _pick(ORDER_STATUSES, rand)

        item_count = 1 + int(math.floor(rand() * 5))
        items: list[dict] = []
        total = 0.0
        for _j in range(item_count):
            item_price = round((10 + rand() * 200) * 100) / 100
            qty = 1 + int(math.floor(rand() * 3))
            total += item_price * qty
            items.append(
                {
                    "productId": f"prod-{str(1 + int(math.floor(rand() * 30))).zfill(4)}",
        ...
        state_idx = int(math.floor(rand() * len(STATES)))
        address = {
            "street": f"{100 + int(math.floor(rand() * 9900))} {_pick(STREET_NAMES, rand)}",
            "city": CITIES[state_idx],
            "state": STATES[state_idx],
            "zip": str(10000 + int(math.floor(rand() * 89999))),
            "country": "US",
        }
        ...
        records.append(
            AerospikeRecord(
                ...
                meta=RecordMeta(
                    generation=1 + int(math.floor(rand() * 3)),
                    ttl=86400 * 90,
                    lastUpdateMs=now_ms - int(math.floor(rand() * 86400000 * 30)),
                ),
                bins={
