...
describe("usePagination", () => {
  it("calculates totalPages correctly", () => {
    const { result } = renderHook(() =>
      usePagination({ total: 100, page: 1, pageSize: 25 }),
    );
  ...
  it("returns at least 1 page for empty results", () => {
    const { result } = renderHook(() =>
      usePagination({ total: 0, page: 1, pageSize: 25 }),
    );
  ...
  it("calculates start and end indices", () => {
    const { result } = renderHook(() =>
      usePagination({ total: 100, page: 2, pageSize: 25 }),
    );
  ...
  it("caps end at total for last page", () => {
    const { result } = renderHook(() =>
      usePagination({ total: 30, page: 2, pageSize: 25 }),
    );
  ...
  it("hasNext is false on last page", () => {
    const { result } = renderHook(() =>
      usePagination({ total: 100, page: 4, pageSize: 25 }),
    );
  ...
  it("generates correct pages array", () => {
    const { result } = renderHook(() =>
      usePagination({ total: 75, page: 1, pageSize: 25 }),
    );
