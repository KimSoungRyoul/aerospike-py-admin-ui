...
export function VirtualDataTable<TData, TValue>({
  ...
  const estimatedRowHeight = densityRowHeight[density];
  ...
  // Render header (shared between virtual and non-virtual)
  const renderHeader = () =>
  ...
  // Render a single row (shared between virtual and non-virtual)
  const renderRow = (row: Row<TData>, idx: number, style?: React.CSSProperties) => (
    <tr
      key={row.id}
      className={cn(
      ...
      style={{
        ...(virtualScrolling ? {} : { animationDelay: `${idx * 25}ms` }),
        ...style,
      }}
  ...
  // Non-virtual rendering (same as DataTable)
  if (!virtualScrolling) {
    return (
      <div className={cn("relative flex-1 overflow-auto", className)} data-testid={testId}>
