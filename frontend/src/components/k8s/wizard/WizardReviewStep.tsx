                ...
                {ns.storageEngine.type === "device"
                  ? `Persistent (${
                      form.storage && "volumes" in form.storage
                        ? form.storage.volumes.find((v) => v.source === "persistentVolume")?.persistentVolume?.size || "10Gi"
                ...
                <div key={vi} className="ml-2 rounded border p-1.5 text-xs">
                  ...
                  {vol.source === "persistentVolume" && vol.persistentVolume && (
                    <span className="text-muted-foreground ml-1">
                      ...
                      {vol.persistentVolume.storageClass ? ` (${vol.persistentVolume.storageClass})` : ""}
                    </span>
                  )}
                  {vol.aerospike?.path && (
                    <span className="text-muted-foreground ml-1">
                      {" "}at {vol.aerospike.path}
                    </span>
                  )}
          ...
          <>
            ...
            <span className="font-medium">
              {editable ? (
                ...
                <>
                  {(form.storage as { size: string }).size} ({(form.storage as { storageClass: string }).storageClass})
                </>
              )}
              {(form.storage as { initMethod?: string }).initMethod ? `, init: ${(form.storage as { initMethod: string }).initMethod}` : ""}
              {(form.storage as { wipeMethod?: string }).wipeMethod ? `, wipe: ${(form.storage as { wipeMethod: string }).wipeMethod}` : ""}
              {(form.storage as { cascadeDelete?: boolean }).cascadeDelete === false ? ", no cascade delete" : ""}
            </span>
          </>
        ) : null}
